...
def get_last_result_insight(draws):
    ...
    if os.path.exists('data/base_super.txt'):
        base_digits = load_base_from_file('data/base_super.txt')
        insight_lines.append("✅ Menggunakan **Super Base** sebagai asas.")
    elif os.path.exists('data/base_last.txt'):
        base_digits = load_base_from_file('data/base_last.txt')
        insight_lines.append("ℹ️ Menggunakan **Base Terkini** sebagai asas.")
    else:
        insight_lines.append("⚠️ Tiada base dijumpai.")
        return '\n'.join(insight_lines)
...
if st.button("📥 Update Draw Terkini"):
    ...
    save_base_to_file(base_digits, 'data/base_last.txt')
    st.markdown("### 📋 Base Terkini (Salin & Tampal)")
    st.code(display_base_as_text('data/base_last.txt'), language='text')
