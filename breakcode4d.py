...
def generate_combinations(base_digits):
    all_combos = []
    for a in base_digits[0]:
        for b in base_digits[1]:
            for c in base_digits[2]:
                for d in base_digits[3]:
                    combo = f"{a}{b}{c}{d}"
...
def get_last_result_insight(draws, limit=30):
    ...
    all_numbers = [d["number"] for d in draws[-limit:] if len(d["number"]) == 4]
...
def ai_tuner_suggestion(draws):
    return "🧠 AI Tuner: Sistem akan melaraskan filter dan scoring secara automatik (simulasi)."
...
if st.button("🚀 Jana Ramalan Terkini"):
    ...
    for i, r in enumerate(ramalan):
        if i < 5:
            colA.markdown(f"**{i+1:02d}**: `{r}`")
        else:
            colB.markdown(f"**{i+1:02d}**: `{r}`")